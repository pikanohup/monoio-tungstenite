...
impl FrameHeader {
    ...
    /// Parses a header from an input stream.
    ///
    ...
    /// Generates a random frame mask and store this in the header.
    ///
...
impl Frame {
    ...
    /// Gets a mutable reference to the frame's header.
    #[inline]
    pub fn header_mut(&mut self) -> &mut FrameHeader {
        &mut self.header
    }

    /// Gets a reference to the frame's payload.
    #[inline]
    pub fn payload(&self) -> &[u8] {
        &self.payload
    }

    /// Tests whether the frame is masked.
    #[inline]
    pub(crate) fn is_masked(&self) -> bool {
        self.header.mask.is_some()
    }

    /// Generates a random mask for the frame.
    ///
    ...
    /// Consumes the frame into its payload.
    #[inline]
    ...
    /// Creates a new Pong control frame.
    #[inline]
    ...
    /// Creates a new Ping control frame.
    #[inline]
    ...
    /// Creates a frame from given header and data.
    pub fn from_payload(header: FrameHeader, payload: Bytes) -> Self {
        Frame { header, payload }
    }

    /// Writes a frame out to a buffer
...
impl LengthFormat {
    /// Gets the length format for a given data size.
    #[inline]
    pub(super) fn for_length(length: u64) -> Self {
        if length < 126 {
    ...
    /// Gets the size of the length encoding.
    #[inline]
    ...
    #[inline]
    pub(super) fn length_byte(&self) -> u8 {
        match *self {
            LengthFormat::U8(b) => b,
    ...
    /// Gets the length format for a given length byte.
    #[inline]
    pub(super) fn for_byte(byte: u8) -> Self {
        match byte & 0x7F {
